...
- **Stage 2b 進行中**(把 `src/main.js` 逐塊抽成 ES module):
  ...
  - 增量15 ✅ `src/ui.js` — HUD、選卡、暫停、設定、轉世閣、商城與角色選擇介面；玩法 mutation 透過 hooks 保留在 authoritative runtime/main。
  - 增量16 ✅ `src/boot.js` — 固定 60Hz 迴圈、繪圖節流、自適應 DPR、頁面／Canvas／按鍵／按鈕事件與啟動入口；`start()` 等玩法狀態轉換保留在 main。
