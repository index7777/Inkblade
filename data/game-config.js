...
(function installInkBladeConfig(global) {
  ...
  const OP_SCHEMA = Object.freeze({
    ...
    flag: ['flags.firstStrikeCrit', 'flags.returnLeavesDryBrush', 'flags.splashOnKill', 'flags.listenToInk', 'flags.whiteCutOnCrit',
      ...
      'flags.freeCastAtFull', 'flags.autoRefill', 'flags.focusStacks', 'flags.focusStrike',
      ...
      'flags.pierceRamp', 'flags.pierceBreak', 'flags.pierceRecoil', 'flags.fullPierce',
      ...
      'flags.afterimageSolid', 'flags.anchorField', 'flags.anchorLink', 'flags.anchorDetonate', 'flags.beadSlow'],
    status: ['erosion', 'suppression', 'whitecut'],
  ...
  // maxBonus 已取消上限:劍意上限越高 → 畫得越長 → 耐久越多,一條鏈到底。
  // 原本封頂 12(1440 寸),納息滿階或靈府初成點滿的玩家會撞到,
  // 等於「每多畫一寸都在付錢買零」,把投資劍意上限的意義砍掉一半。
  const LIFE_MODEL = Object.freeze({ pixelsPerLife: 120, maxBonus: Infinity });
  ...
  // 劍速轉傷害:與主檔 speedMul() 同一組數字,卡面才算得出來
  const SPEED_MODEL = Object.freeze({ baseSpeed: 14, damagePerSpeed: 0.6 });
  // ── 劍寬的第二層意義 ──────────────────────────────────────────
  // 寬 → 護甲高(厚重不易崩)、飛得慢;細 → 護甲低、飛得快。
  // 兩者**都不影響傷害** —— 傷害只由劍傷本身與「劍速卡片」決定。
  // (劍寬本來就已經在推高每寸劍意成本,見 COST_MODEL,這裡是它的正面回報。)
  // 護甲曲線刻意不對稱:加寬給的護甲多、收細扣的護甲少。
  // 對稱的話收細會被扣到每擊 2.0,把「畫得長」的優勢全部吃掉(實測斂鋒滿階只剩 1.07 倍)。
  const WIDTH_MODEL = Object.freeze({ baseWidth: 18, armorPerWidth: 0.70, armorPerWidthThin: 0.25,
                                      speedPerWidth: 0.18, minFlySpeed: 6 });
  ...
  const LEGACY_INSIGHTS = Object.freeze([
    ...
    // ── 劍勢:改變飛劍運動。 ───────────────────────────────────────
    insight({ id: 'momentum_swift', category: CATEGORY.MOMENTUM, formationLock: 'fan', rarity: RARITY.AWAKENING, name: '疾影', rune: '疾', maxRank: 5, description: '散鋒專屬；飛劍加速，修成殘影追斬', tiers: [tier(0, '飛劍身後留下兩道殘影。', [op('flag', 'flags.afterimage', true)]), tier(1, '殘影對原目標追加該擊25%傷害。', [op('flag', 'flags.afterimageHits', true)]), tier(2, '殘影改為命中附近其他墨獸。', [op('flag', 'flags.afterimageSolid', true)])], effects: [op('add', 'stats.swordSpeed', 3.6)], fx: { trail: fx.DRY_BRUSH } }),
    insight({ id: 'momentum_guide', category: CATEGORY.MOMENTUM, formationLock: 'parallel', rarity: RARITY.PENETRATION, name: '引鋒', rune: '引', maxRank: 5, description: '齊鋒專屬；續飛追敵，傷−12%、不暴擊', tradeoff: '每次領悟令直擊傷害降低 12%,引鋒本身不能暴擊。', requires: ['inherit_guide'], tiers: [tier(0, '劍令末端的延伸更長。', [op('flag', 'flags.guideExtend', true)]), tier(1, '擊殺後立即再向下一目標延伸。', [op('flag', 'flags.guideRetarget', true)]), tier(2, '劍令絕不空走,必尋得墨獸。', [op('flag', 'flags.guideNeverMiss', true)])], effects: [op('add', 'mechanics.homingStrength', 0.055), op('mul', 'stats.damage', 0.88), op('set', 'mechanics.homingCanCrit', false)], fx: { trail: fx.DRY_BRUSH } }),
    insight({ id: 'momentum_return', category: CATEGORY.MOMENTUM, formationLock: 'merge', rarity: RARITY.PENETRATION, name: '歸鋒', rune: '歸', maxRank: 5, description: '聚鋒專屬；每階增加1次邊界折返', tiers: [tier(0, '回程速度提高三成。', [op('flag', 'flags.returnFaster', true)]), tier(1, '回程末端自行探向最近墨獸。', [op('flag', 'flags.returnSeek', true)]), tier(2, '歸而不竭:每次折返保留三成耐久。', [op('flag', 'flags.returnKeep', true)])], effects: [op('set', 'mechanics.returnEnabled', true), op('add', 'mechanics.returnHits', 1)], fx: { trail: fx.DRY_BRUSH, hit: fx.WHITE_CUT } }),
    insight({ id: 'momentum_break', category: CATEGORY.INTENT, rarity: RARITY.PENETRATION, name: '墨痕', rune: '墨', maxRank: 5, description: '命中爆墨；每階半徑+38、波及傷+12%', tradeoff: '潑墨只在命中時出現,不改變飛劍本體。', tiers: [tier(0, '潑墨範圍內留下墨滴。', [op('flag', 'flags.inkDropOnSplash', true)]), tier(1, '墨滴可再次炸開。', [op('flag', 'flags.inkDropExplode', true)]), tier(2, '潑墨可連鎖引動。', [op('add', 'mechanics.splashChain', 1)])], effects: [op('add', 'mechanics.splashRadius', 38), op('mul', 'mechanics.splashDamage', 1.12)], fx: { hit: fx.SPLASH } }),
    insight({ id: 'momentum_anchor', category: CATEGORY.MOMENTUM, formationLock: 'inline', rarity: RARITY.PENETRATION, name: '定鋒', rune: '定', maxRank: 5, description: '貫鋒專屬；耐久耗盡插地持續斬擊', tradeoff: '串珠鏈內飛劍移速 ×0.92;插地後不再移動、脫離隊列。', tiers: [tier(0, '定鋒墨域:劍樁生成減速墨氣領域,經過的串珠回補穿透。', [op('flag', 'flags.anchorField', true)]), tier(1, '鋒引相連:鄰近劍樁生成墨鏈,鏈上敵受切傷,穿鏈本擊 +22%。', [op('flag', 'flags.anchorLink', true)]), tier(2, '鋒殼盡碎:劍樁壽終引爆,觸發墨痕全套;可被手繪劍跡主動引爆。', [op('flag', 'flags.anchorDetonate', true)])], effects: [op('add', 'mechanics.anchorDuration', 0.25), op('add', 'mechanics.anchorDamageMult', 0.08), op('flag', 'flags.beadSlow', true)], fx: { hit: fx.BREAK_INK } }),
    // ── 劍意:命中後留在墨獸身上的狀態。 ──────────────────────────
    insight({ id: 'intent_erosion', category: CATEGORY.INTENT, rarity: RARITY.CLARITY, name: '蝕痕', rune: '蝕', maxRank: 5, description: '命中疊蝕；每層每秒4傷，最多6層', tiers: [tier(0, '蝕痕可被重新刷新。', [op('flag', 'flags.dotRefresh', true)]), tier(1, '蝕痕向鄰近墨獸擴散。', [op('flag', 'flags.dotSpread', true)]), tier(2, '墨獸潰散時爆出剩餘蝕傷。', [op('flag', 'flags.dotBurst', true)])], effects: [status('erosion', { stacks: 1, damagePerSecond: 4, duration: 3.2, maxStacks: 6 })], fx: { hit: fx.WHITE_CUT, status: fx.ERODING_INK } }),
    insight({ id: 'intent_suppress', category: CATEGORY.INTENT, rarity: RARITY.CLARITY, name: '鎮痕', rune: '鎮', maxRank: 5, description: '命中鎮痕；緩速12%、每秒1.5傷', tiers: [tier(0, '鎮痕緩速幅度提高(+0.06)。', [op('add', 'mechanics.slowBonus', 0.06)]), tier(1, '鎮痕額外定身三分之一息。', [op('flag', 'flags.rootOnSuppress', true)]), tier(2, '定身結束時炸開一道留白斬。', [op('flag', 'flags.rootWhiteCut', true)])], effects: [status('suppression', { stacks: 1, slow: 0.12, damagePerSecond: 1.5, duration: 2.6, maxStacks: 4 })], fx: { hit: fx.NEGATIVE_SPACE, status: fx.SUPPRESSED_INK } }),
  ...
  const LEGACY_REBIRTH = Object.freeze([
    ...
    rebirthNode({ id: 'foundation_sword_bone', branch: REBIRTH_BRANCH.FOUNDATION, name: '劍骨凝成', description: '每階令開局劍傷提高三。', maxRank: 10, costs: [30, 65, 125, 220, 340, 490, 680, 910, 1180, 1500], effects: [op('add', 'stats.damage', 3)] }),
    rebirthNode({ id: 'foundation_flow', branch: REBIRTH_BRANCH.FOUNDATION, name: '行氣如劍', description: '每階令開局劍速提高一。', maxRank: 10, costs: [40, 95, 190, 330, 510, 730, 1000, 1320, 1690, 2120], effects: [op('add', 'stats.swordSpeed', 1)] }),
    rebirthNode({ id: 'foundation_cycle', branch: REBIRTH_BRANCH.FOUNDATION, name: '周天養息', description: '每階令劍意回復提高。', maxRank: 10, costs: [40, 95, 190, 330, 510, 730, 1000, 1320, 1690, 2120], effects: [op('add', 'stats.manaRegen', 0.05)] }),
  ...
  function applyInsight(state, insightId) {
    ...
    if (Array.isArray(state.history)) state.history.push(item.id);
  ...
  const EFFECT_LABEL = Object.freeze({
    ...
    'mechanics.homingStrength':'追蹤','mechanics.returnHits':'折返趟數','mechanics.splashRadius':'潑墨範圍',
  ...
  const FORMATION_CN = Object.freeze({ fan:'扇形', parallel:'平行', inline:'連珠', merge:'聚形' });
  ...
  function opLines(effects){
    ...
    for(const e of (effects||[])){
      ...
      if(e.op==='flag'){ const t=FLAG_LABEL[e.path];
        ...
        // 劍速會轉成傷害(主檔 speedMul:以 14 為基準,每快一成加 0.6 成傷害)。
        // 只寫「劍速 +3.6」玩家算不出那是多少輸出,所以直接把換算標在同一行。
        if(e.path==='stats.swordSpeed'){
          const d=Math.round(v/SPEED_MODEL.baseSpeed*SPEED_MODEL.damagePerSpeed*100);
          if(d) line+=' 傷'+(d>0?'+':'−')+Math.abs(d)+'%';
        }
        out.push(line);
      } else if(e.op==='mul' && L){
  ...
  // 換陣重算:以領悟歷史重放,跳過與「當前劍陣」不符的劍行(停用其加成;切回該陣再恢復)。
  // 需由主檔在「換陣(領悟新劍式導致 formation 改變)」後呼叫。
  function recomputeForFormation(state) {
    const hist = Array.isArray(state.history) ? state.history.slice() : [];
    const keepTimes = state.resetInsightTimes || 0;
    const keepKills = { ...(state.tierKills || {}) };
    const keepLevel = { ...(state.tierLevel || {}) };
    const snap = state.permanentSnapshot || { permanentUnlocks: state.permanentUnlocks || [] };
    const diff = state.difficultyScale || 1;
    let cur = 'single';
    for (const id of hist) {
      const it = insightById[id];
      if (it && it.category === CATEGORY.FORM) {
        const f = it.effects.find(e => e.op === 'set' && e.path === 'formation');
        if (f) cur = f.value;
      }
    }
    const fresh = createRunState(snap, diff);
    Object.keys(state).forEach(k => { delete state[k]; });
    Object.assign(state, fresh);
    state.resetInsightTimes = keepTimes;
    state.history = [];
    for (const id of hist) {
      const item = insightById[id];
      if (!item) continue;
      state.history.push(id);
      state.ranks[id] = Number(state.ranks[id] || 0) + 1;
      if (item.formationLock && item.formationLock !== cur) continue; // 停用:與當前劍陣不符
      applyInsightEffects(state, item);
    }
    state.tierKills = keepKills;
    state.tierLevel = keepLevel;
    clampAllStats(state);
    return state;
  }
