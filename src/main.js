...
function renderSwordArts(){
  ...
  list.innerHTML=Object.keys(groups).map(cat=>'<section class="artgroup"><h3>'+ART_CATEGORY_NAME[cat]+'</h3>'+
    groups[cat].map(a=>{ const raw=Number(runState.ranks[a.id]||0), stage=Math.min(3,Math.max(1,Math.ceil(raw/Math.max(1,a.maxRank||1)*3)));
      return '<div class="artrow"><b>'+a.name+'</b><span>'+artTierName(stage)+'</span></div>'; }).join('')+'</section>').join('');
}
